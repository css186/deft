...
DEFINE_int32(delete_ratio, 0, "delete ratio");
...
void thread_run(int id) {
  ...
  for (int i = 0; i < NUM_WARMUP_OPS; ++i) {
    ...
    else if (rand_r(&seed) % 100 < FLAGS_read_ratio + FLAGS_delete_ratio) {
        tree->del(key);
    }
    else {
      v = 12;
  ...
  for (int i = 0; i < FLAGS_ops_per_thread; ++i) {
    ...
    else if (rand_r(&seed) % 100 < FLAGS_read_ratio + FLAGS_delete_ratio) { // DELETE
      tree -> del(key);
    }
    else {
      v = 12;
...
void print_args() {
  printf(
      ...
      "%d, ReadRatio %d, DeleteRatio %d"
      "Zipfan %.3lf\n",
      FLAGS_server_count, FLAGS_client_count, FLAGS_num_prefill_threads,
      FLAGS_num_bench_threads, FLAGS_read_ratio, FLAGS_delete_ratio, FLAGS_zipf);
}
