...
with open(file_name, 'w') as fp:
    ...
    for job_id, (key_space, read_ratio, zipf, num_threads) in enumerate( product_list):
        ...
        for i in range(num_servers):
            ...
            cmd = f'cd {exe_path} && sudo sh -c "echo 3 > /proc/sys/vm/drop_caches" && numactl --membind=8-15 --cpunodebind=8-15 ./{g_cfg["server_app"]} --server_count {num_servers} --client_count {num_clients} --numa_id {numa_id} &> ../log/server_{i}.log'            
        ...
        for i in range(num_clients):
            ...
            cmd = f'cd {exe_path} && numactl --membind=8-15 --cpunodebind=8-15 ./{g_cfg["client_app"]} --server_count {num_servers} --client_count {num_clients} --numa_id {numa_id} --num_prefill_threads {num_prefill_threads} --num_bench_threads {num_threads} --key_space {key_space} --read_ratio {read_ratio} --zipf {zipf} &> ../log/client_{i}.log'
            ssh, stdin, stdout, stderr = ssh_command(ip, username, password, cmd)
